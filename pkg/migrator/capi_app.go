	...
	data := &ClusterAppValuesData{
		...
		Internal: Internal{
			Migration: Migration{
				...
				IrsaAdditionalDomain: fmt.Sprintf("https://irsa.%s.k8s.%s.%s.aws.gigantic.io", s.clusterInfo.Name, s.clusterInfo.MC.VintageMC, s.vintageCRs.AwsCluster.Spec.Provider.Region),
			},
		},
	}
